package dev.thomazz.pledge.sponge.network;

import dev.thomazz.pledge.sponge.api.event.PacketFlushEvent;
import dev.thomazz.pledge.sponge.api.event.PacketFrameSendEvent;
import dev.thomazz.pledge.sponge.PlayerHandlerImpl;
...
public class PacketFrameOutboundTailHandler extends ChannelOutboundHandlerAdapter {
    ...
    public void drain(ChannelHandlerContext ctx, @Nullable PacketFrame frame) throws Exception {
        ...
        if (frame != null) {
            ...
            Sponge.eventManager().post(new PacketFrameSendEvent(player, frame));
        }
